...
const Signup = ({...props}) => {
  const [customer, setCustomer] = useState({
    ...
    if(modalProp){
      setHooray({title:'true',heading:"It seems we don't deliver at your address yet"});
            ...
            axios
              ...
              .then((res) => {
                ...
                if (res.data.code === 200) {
                  ...
                  setApiCode1({title:"Account already Exist ",subject:"We have found this account with a different login. Please click below to continue."});
...
if(apiCode===480){
  setApiCode1("There Seems to be a problem on our side.We are working on it.")
}
  ...
  return (
    ...
    >
      ...
      >
        ...
        ></Box>
        <p style={{ fontWeight: 'bold' }} onClick={onSubmit3}>Or continue with email</p>
        ...
        {emailError && <Box><p style={{color:'red'}}>Emails don't match</p></Box>}
        <Box mb={1} style={{ marginBottom: '20px' }}>
          <CssTextField
        ...
        {passError && <Box><p style={{color:'red'}}>Passwords don't match</p></Box>}
        <Button
            variant="contained"
            type="submit"
            onClick={onSubmit2}
      ...
      {apiCode && <AlertModal title={apiCode1.title} subject={apiCode1.subject}clear={setApiCode} signClear={props.setIsSignUpShown} loginShow={props.setIsLoginShown}></AlertModal>}
      {hooray && <Hooray  clear={setHooray} signClear={props.setIsSignUpShown} ok={setModal}></Hooray>}
    </Paper>
  );
};
