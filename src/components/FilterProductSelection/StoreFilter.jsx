...
const StoreFilter = () => {
  ...
  const getDeliveryDate = () => {
    ...
    const deadlineInfo = dayUpper
      ? store.acceptDayHour[dayUpper].split(' ')
      : '';
    ...
    for (const fullDay of fullDays) {
      if (
        fullDay.substring(0, dayShortUpper.length).toUpperCase() ==
        dayShortUpper
      ) {
        day = fullDay;
      }
  ...
  return (
    <FilterContext.Provider value={{ shownDays }}>
      <Box className={classes.storeFilterContainer}>
        <Typography
          style={{ display: 'flex' }}
          className={clsx(classes.deliveryDates, classes.filterCol)}
        >
        ...
        <Box className={classes.filterItemsGrid} mb={1}>
          <Box display="flex">{ItemStack(DaysCategory)}</Box>
          <Box
            display="flex"
            flexDirection="row"
